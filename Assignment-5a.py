...
import numpy as np
### import dataset
df=pd.read_csv('D:/data science/assignments_csv/50_Startups.csv')
df
### Renaming the variabels
df1=df.rename({'R&D Spend':'RDS','Administration':'ADMS','Marketing Spend':'MKS'},axis=1)
df1
...
## scatterplot between ADMS and profit
import matplotlib.pyplot as plt
plt.scatter(x=df1[['ADMS']],y=df1['Profit'],color='red')
plt.xlabel('ADMS')
plt.ylabel('Profit')
plt.show()
df1['ADMS'].hist() ## ADMS histogram
df1.boxplot(column='ADMS',vert=False) ## ADMS Box plot
...
### by taking R2 Value  Model-4 best 
### Multicoliniarity for model-4 fit

import statsmodels.formula.api as smf
model=smf.ols('Profit~ADMS+MKS',data=df1).fit()
model.summary()

import statsmodels.formula.api as smf
model = smf.ols('ADMS~MKS',data=df1).fit()
R2 = model.rsquared
VIF = 1/(1-R2)
...
####### lable encoding on State variable which is Catagorical datatype
from sklearn.preprocessing import LabelEncoder
LE = LabelEncoder()
df1["State"] = LE.fit_transform(df1["State"])
df1["State"]

## StandarScalar on ADMS,MKS which is a continious datatype
df1_cont = df1[['ADMS','MKS']]
from sklearn.preprocessing import StandardScaler
SS = StandardScaler()
SS_cont    = SS.fit_transform(df1_cont)
SS_cont  = pd.DataFrame(SS_cont)
SS_cont.columns= ['ADMS','MKS']
SS_cont
